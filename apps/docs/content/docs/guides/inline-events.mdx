---
title: Inline event handlers for Starting Point UI components
description: React to component events straight from your markup with data-sp-on attributes, no script files needed. Ideal for server-rendered stacks like Laravel and Rails.
---

Inline events let you react to the components' own events right in your HTML, like showing a toast when a dialog opens, without a script file for every small interaction. They are made for server-rendered stacks like Laravel Blade or Rails, where the markup is right there. This feature is currently experimental.
