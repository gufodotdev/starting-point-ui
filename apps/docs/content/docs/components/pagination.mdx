---
title: Tailwind CSS Pagination
description: Beautiful, modern pagination component for Tailwind CSS. Use it to move between pages of content, like a data table or search results. Easily customizable.
---

Use the pagination component to move between pages of content, like a data table, search results, or a blog archive. For paging the slides of a carousel, use its own [carousel pagination](/components/carousel#carousel-pagination).
...
| `pagination-item`                       | Add to each `<a>`, with `active` on the current page     |
...
Use only the previous and next buttons, like next to a rows per page [select](/components/select) in a data table.
...
Add `dir="rtl"` to any parent element, typically `<html>`, to render right-to-left. Flip the previous and next chevrons with `rtl:rotate-180`. See the [RTL guide](/guides/rtl) for details.
