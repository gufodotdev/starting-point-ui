...
Use the combobox component to pair a text field with a filterable list of options, like a country picker, a timezone field, or a tag input with chips. For a short static set of options, reach for the [select](/components/select), and for a menu of actions, the [dropdown](/components/dropdown).
