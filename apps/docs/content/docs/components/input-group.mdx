---
title: Tailwind CSS Input Group
description: Beautiful, modern input group component for Tailwind CSS. Use it to put icons, text, and buttons in one field, like a search box. Easily customizable.
---

Use the input group component to put icons, text, and buttons inside one bordered field, like a search box with its icon or a price with its currency. For attaching full-size buttons next to a field, reach for the [button group](/components/button-group).
...
Add the `input-group` class to the wrapper and put a regular [input](/components/input) or [textarea](/components/textarea) inside. The wrapper takes over the border, focus ring, and invalid state for the whole field. Put extras in `input-group-addon` elements, where regular [buttons](/components/button) shrink to fit the field.

| Class                           | Description                                                               |
| ------------------------------- | ------------------------------------------------------------------------- |
| `input-group`                   | Add to the wrapper, which draws the border, focus ring, and invalid state |
| `input-group-sm`                | Add to `input-group` for a compact field                                  |
...
Put regular [buttons](/components/button) in an addon and they shrink to fit the field, and a [popover](/components/popover) or any other overlay can hang off them.
