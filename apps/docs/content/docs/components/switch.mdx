...
Pair with a [Label](/components/label) for accessible form fields. Connect them using matching `for` and `id` attributes so clicking the label toggles the switch and screen readers announce it correctly.
