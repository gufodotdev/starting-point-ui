---
title: Tailwind CSS Chat Message
description: Beautiful, modern chat message component for Tailwind CSS. Use it to lay out one row of a chat, like in a messaging app or an AI assistant. Easily customizable.
---

Use the message component to lay out one row of a chat, like in a messaging app or an AI assistant. Fill it with [bubbles](/components/bubble) and scroll the rows in a [message scroller](/components/message-scroller).
...
Wrap consecutive rows from the same sender in `message-group`, and an empty `message-avatar` keeps the rows aligned.
...
Put [buttons](/components/button) in `message-footer` for per-message actions, and give icon-only ones an `aria-label`.
