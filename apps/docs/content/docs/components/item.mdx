---
title: Tailwind CSS List Item
description: Beautiful, modern list item component for Tailwind CSS. Use it to lay out a row with media, text, and actions, like in a list or a menu. Easily customizable.
---

Use the item component to lay out a row with a media slot, text, and trailing actions, like in a list, a menu, or a settings page. For a standalone content block, reach for the [card](/components/card), and for a similar-looking row that delivers a message, the [alert](/components/alert).
...
| `item-group`          | Add to a wrapper stacking items, with `role="list"`   |
...
```html preview preset=base
<div class="item-group w-full max-w-md" role="list">
...
```html preview preset=base
<div class="item-group grid w-full max-w-md grid-cols-2 gap-3 sm:gap-4" role="list">
  <div class="item item-outline">
...
An item on an `<a>` picks up a hover style. Mark external links with an icon.
...
```html preview preset=base align=top open=.dropdown
...
<div id="item-rtl-demo" dir="rtl" class="flex w-full max-w-md flex-col gap-4">
  <div class="item item-outline">
    <div class="item-content">
