...
description: Beautiful, modern accordion component for Tailwind CSS. Use it to show and hide content sections, like an FAQ or a settings panel. Easily customizable.
---

Use the accordion component to stack sections that expand and collapse in place, like the questions of an FAQ or the steps of a setup guide. For a single independent section, use a [collapsible](/components/collapsible).
...
| Class               | Description                                                    |
| ------------------- | -------------------------------------------------------------- |
| `accordion`         | Add to the wrapper, which carries the `data-sp-*` options      |
| `accordion-item`    | Add to each section, drawing the divider to the next one       |
| `accordion-trigger` | Add to the `<button>`, wrapped in a heading                    |
| `accordion-panel`   | Add to the collapsing element, where `expanded` starts it open |
