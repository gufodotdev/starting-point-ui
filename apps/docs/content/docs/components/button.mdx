...
description: Beautiful, modern button component for Tailwind CSS. Use it to trigger actions, like in forms, dialogs, and toolbars. Easily customizable.
---

Use the button component to trigger actions like submitting a form, confirming a decision, or opening a dialog. For a set of related actions, attach several into a [button group](/components/button-group).
...
| `btn-icon`                                                             | Add to `btn` for a square icon-only button, in any size            |
| `btn-badge`                                                            | Add to a `<span>` inside the button for a corner count             |
...
Add a `<span>` with `btn-badge` inside the button for a corner count, and recolor it with background utilities like `bg-destructive`.
