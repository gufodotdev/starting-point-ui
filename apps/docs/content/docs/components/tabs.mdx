...
description: Beautiful, modern tabs component for Tailwind CSS. Use it to show one section at a time behind a row of tabs, like a settings screen. Easily customizable.
---

Use the tabs component to show one section at a time behind a row of tabs, like the pages of a settings screen or a product's details. For content that should stay visible and stack, use an [accordion](/components/accordion).
...
| `tab`           | Add to each `<button>`, with `active` on the selected one |
| `tab-content`   | Add to each panel, with `active` on the visible one       |
