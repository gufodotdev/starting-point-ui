---
title: Carousel
description: A carousel slider component for Tailwind CSS with beautiful shadcn/ui styling. Use it for image galleries, product showcases, testimonials, or hero sections.
---

The carousel cycles through a series of slides in place. Use it for image galleries, product showcases, testimonials, or hero sections; for sections readers pick by name instead of browsing, reach for [tabs](/components/tabs).

```html preview preset=carousel
...
Add the `carousel` class to a container. Inside it, add a `carousel-content` element with one `carousel-item` per slide. For controls, put `carousel-previous` and `carousel-next` on [buttons](/components/button) anywhere inside the container and style them however you like.
...
```html preview preset=carousel dir=rtl
...
| ---------------- | ------- | ---------------------------------------------- |
| `--carousel-gap` | `1rem`  | Gap between slides; the fitted sizes follow it |
