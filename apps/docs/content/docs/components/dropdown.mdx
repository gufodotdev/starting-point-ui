---
title: Tailwind CSS Dropdown Menu
description: Beautiful, modern dropdown menu component for Tailwind CSS. Use it to show a menu of actions, like a user menu or row actions. Easily customizable.
---

Use the dropdown component to show a menu of actions behind a trigger, like a user menu or the commands behind a "more" button. For rich free-form content, reach for the [popover](/components/popover), and for picking a value in a form, the [select](/components/select) or [combobox](/components/combobox).
...
| Class                       | Description                                                                             |
| --------------------------- | --------------------------------------------------------------------------------------- |
| `dropdown`                  | Add to the menu panel, which carries the `data-sp-*` options                            |
...
| `dropdown-item-radio`       | Add to `dropdown-item` wrapping a hidden `<input type="radio">` with a shared `name`    |
...
Set where the menu opens relative to its trigger with `data-sp-placement`. It accepts any [Floating UI placement](https://floating-ui.com/docs/computeposition#placement), like `top`, `right-start`, or `bottom-end`, plus the logical sides `inline-start` and `inline-end` that follow the text direction. The menu flips to the opposite side on its own when there isn't room.
...
```html preview preset=base align=top open=.dropdown dir=rtl
