---
title: Tailwind CSS Radio Group
description: Beautiful, modern radio button component for Tailwind CSS. Use it to pick exactly one option from a set, like a plan or a shipping method. Easily customizable.
---

Use the radio group component to pick exactly one option from a set of radio buttons, like a plan or a shipping method. For on and off settings, reach for the [checkbox](/components/checkbox) or [switch](/components/switch).

```html preview preset=sm
<div class="field-group w-full max-w-sm gap-3" role="radiogroup" aria-label="Density">
...
Add the `radio` class to each `<input type="radio">` and give the set a shared `name`. Lay each option out with its label in a horizontal [field](/components/field), and name the group with a `field-set` and legend or with `role="radiogroup"` and an `aria-label`.
...
<div class="field-group w-full max-w-sm gap-3" role="radiogroup" aria-label="Plan">
...
```html preview preset=sm
<div class="field-group w-full max-w-sm gap-3" role="radiogroup" aria-label="Options">
  <div class="field field-horizontal">
...
<div id="radio-group-rtl-demo" dir="rtl" class="w-full max-w-sm">
