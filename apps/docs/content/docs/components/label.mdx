...
Connect a label to an input using matching `for` and `id` attributes. Clicking the label focuses the input, and screen readers announce it correctly. Use a [Field](/components/field) wrapper for consistent spacing.
