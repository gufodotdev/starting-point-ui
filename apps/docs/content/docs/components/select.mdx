...
description: Beautiful, modern select component for Tailwind CSS. Use it to pick one option from a native dropdown, like a category or a timezone. Easily customizable.
---

Use the select component to pick one option from the browser's native dropdown, like a category, a status, or a timezone. Pair it with a [field](/components/field) for a label and description, and for a searchable list, reach for the [combobox](/components/combobox).
