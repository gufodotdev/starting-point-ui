...
Pair with a [Label](/components/label) and wrap in a [Field](/components/field) for accessible form fields. Connect them using matching `for` and `id` attributes so clicking the label focuses the select and screen readers announce it correctly.
...
Combine selects with [avatars](/components/avatar) to assign roles or permissions per user.
