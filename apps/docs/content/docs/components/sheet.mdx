...
| `sheet-start` `sheet-end` `sheet-top` `sheet-bottom` | Add to `sheet` to pick the edge; end is the default. `sheet-left` and `sheet-right` pin a physical edge |
...
```html preview preset=sheet open=.sheet dir=rtl
