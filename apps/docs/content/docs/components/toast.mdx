---
title: Tailwind CSS Toast
description: Beautiful, modern toast notification component for Tailwind CSS. Use it to show a brief message, like a saved confirmation. Easily customizable.
---

Use the toast component to show a brief notification that stacks in a corner and dismisses itself, like "Changes saved" after a form submit. For a message that must be acknowledged, use a [dialog](/components/dialog).
...
Add a clickable action button, and choosing it also dismisses the toast.
...
Control how long the toast stays in milliseconds, default 4000. Pass `0` to keep it until dismissed.
...
Place the stack in any corner or centered at the top or bottom. The default is `bottom-right`.
...
Set `dismissible: false` to hide the close button, and the toast leaves only when its duration expires.
...
Toasts mount on `<body>`, outside any wrapper, so add `dir="rtl"` to the `<html>` element for the stack to follow. See the [RTL guide](/guides/rtl) for details.
...
| `description` | String  | None           | Secondary text under the title                                                        |
| `duration`    | Number  | `4000`         | Milliseconds before auto-dismiss, `0` keeps the toast until dismissed                 |
| `action`      | Object  | None           | `{ label, onClick }` rendered as a button that also dismisses                         |
