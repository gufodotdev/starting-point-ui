---
title: Tailwind CSS Alerts
description: Beautiful, modern alert component for Tailwind CSS. Use it to show a message in place, like a success confirmation, a warning, or an error. Easily customizable.
---

Use the alert component to show a message that stays in place on the page, like a success confirmation, a warning, or an error summary. For a notification that pops up, use the [toast](/components/toast), and for a row that presents content rather than a message, the [item](/components/item).
...
Add the `alert` class to a container with a title and description inside. An svg icon as the first child sits beside the text. Add [`role="alert"`](https://www.w3.org/TR/wai-aria-1.2/#alert) when the message appears dynamically, like a form error after a submit, so screen readers announce it.
