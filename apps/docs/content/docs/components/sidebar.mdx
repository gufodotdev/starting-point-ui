...
<div class="sidebar">
  ...
  <aside class="sidebar-panel" data-collapse="icon" data-sp-toggle="#sidebar-trigger">
...
A sidebar layout pairs a `sidebar` root with a `sidebar-backdrop`, a `sidebar-panel`, and the `sidebar-page` content area. Point the panel at its trigger with `data-sp-toggle`.
...
| `sidebar-backdrop`     | The mobile drawer's dimmed overlay                      |
...
Use `sidebar-static` for a non-collapsing column that embeds anywhere, like a settings [dialog](/docs/components/dialog). It is plain flow layout, so it sits inside bounded containers without the fixed positioning of the full app shell.
...
Wrap it in a [collapsible](/docs/components/collapsible) to make it expandable, and nest them for a tree like a file explorer.
...
Wrap a labelled section of the menu in a `sidebar-group` with a `sidebar-group-label`. Stack multiple groups to organize the menu, and add a `sidebar-group-action` in the label row, for example to open a [dropdown](/docs/components/dropdown).
...
The `sidebar-footer` is a pinned region at the bottom of the panel, often a user row with an [avatar](/docs/components/avatar). Pair it with a [dropdown](/docs/components/dropdown) for an account menu.
