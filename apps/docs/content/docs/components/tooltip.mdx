...
description: Beautiful, modern tooltip component for Tailwind CSS. Use it to show a short text hint on hover, like explaining an icon-only button. Easily customizable.
---

Use the tooltip component to show a short text hint next to its trigger on hover or focus, like explaining an icon-only button. For rich content or anything interactive, use a [popover](/components/popover).
...
| Class     | Description                                                    |
| --------- | -------------------------------------------------------------- |
| `tooltip` | Add to the hint element, which carries the `data-sp-*` options |
...
Set which side the tooltip opens on with `data-sp-placement`, `top` by default. It accepts any [Floating UI placement](https://floating-ui.com/docs/computeposition#placement) and flips to the opposite side on its own when there isn't room.
...
A [kbd](/components/kbd) rides along after the text, and the tooltip tightens its padding for it.
...
Lifecycle events fire on the tooltip element. The `before*` events are cancelable.
